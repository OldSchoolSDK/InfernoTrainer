...
export class TzKalZuk extends Mob {
  ...
  attackIfPossible() {
    ...
    if (this.timerPaused === false) {
      ...
      if (this.setTimer === 0) {
        ...
        const mager = new JalZek(
          this.region,
          { x: 20, y: 21 },
          { aggro: this.shield, spawnDelay: 7 }
        );
        this.region.addMob(mager);
        const ranger = new JalXil(
          this.region,
          { x: 29, y: 21 },
          { aggro: this.shield, spawnDelay: 9 }
        );
  ...
  damageTaken() {
    ...
    if (this.currentStats.hitpoint < 240 && this.enraged === false) {
      ...
      const healer1 = new JalMejJak(
        this.region,
        { x: 16, y: 9 },
        { aggro: this, spawnDelay: 2 }
      );
      this.region.addMob(healer1);

      const healer2 = new JalMejJak(
        this.region,
        { x: 20, y: 9 },
        { aggro: this, spawnDelay: 2 }
      );
  ...
  attack() {
    ...
    if (
      this.aggro.location.x < this.shield.location.x ||
      this.aggro.location.x >= this.shield.location.x + 5
    ) {
