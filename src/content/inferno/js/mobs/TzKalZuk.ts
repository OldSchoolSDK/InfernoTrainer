  ...
  attackIfPossible () {
    ...
    if (this.timerPaused === false) {
      ...
      if (this.setTimer === 0) {
        ...
        const mager = new JalZek(this.world, { x: 20, y: 21}, {aggro: this.shield, spawnDelay: 6})
        this.world.region.addMob(mager);
        const ranger = new JalXil(this.world, { x: 29, y: 21}, {aggro: this.shield, spawnDelay: 8})
  ...
  damageTaken() {
    if (this.timerPaused === false) {
      ...
      if (this.currentStats.hitpoint < 480) {
        ...
        this.timerPaused = false;
        // Spawn Jad
        const jad = new JalTokJad(this.world, { x: 24, y: 25}, { aggro: this.shield, attackSpeed: 8, stun: 1, healers: 3, isZukWave: true, spawnDelay: 9 });
