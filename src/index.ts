...
const waveInput: HTMLInputElement = document.getElementById(
  "waveinput"
) as HTMLInputElement;

const exportWaveInput: HTMLButtonElement = document.getElementById(
  "exportCustomWave"
) as HTMLButtonElement;
const editWaveInput: HTMLButtonElement = document.getElementById(
  "editWave"
) as HTMLButtonElement;
...
editWaveInput.addEventListener("click", () => {
  ...
  const url = `/?wave=0&mager=${JSON.stringify(magers)}&ranger=${JSON.stringify(
    rangers
  )}&melee=${JSON.stringify(meleers)}&blob=${JSON.stringify(
    blobs
  )}&bat=${JSON.stringify(bats)}&copyable`;
...
exportWaveInput.addEventListener("click", () => {
  ...
  const url = `/?wave=74&mager=${JSON.stringify(
    magers
  )}&ranger=${JSON.stringify(rangers)}&melee=${JSON.stringify(
    meleers
...
function importSpawn() {
  try {
    JSON.parse(mager).forEach((spawn: number[]) =>
      selectedRegion.addMob(
        new JalZek(
          selectedRegion,
          { x: spawn[0] + 11, y: spawn[1] + 14 },
          { aggro: player }
        )
      )
    );
    ...
    InfernoWaves.spawnNibblers(3, selectedRegion, randomPillar).forEach(
...
if (selectedRegion.wave === 0) {
  ...
  if (
    ...
    InfernoWaves.spawn(
      ...
      spawns,
      selectedRegion.wave
    ).forEach(selectedRegion.addMob.bind(selectedRegion));
  ...
  selectedRegion.addMob(
    new TzKalZuk(selectedRegion, { x: 22, y: 8 }, { aggro: player })
  );
  ...
  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 14, y: 14 }, "#00FF00", 1, false)
  );

  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 16, y: 14 }, "#FF0000", 1, false)
  );
  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 17, y: 14 }, "#FF0000", 1, false)
  );
  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 18, y: 14 }, "#FF0000", 1, false)
  );

  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 20, y: 14 }, "#00FF00", 1, false)
  );

  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 30, y: 14 }, "#00FF00", 1, false)
  );

  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 32, y: 14 }, "#FF0000", 1, false)
  );
  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 33, y: 14 }, "#FF0000", 1, false)
  );
  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 34, y: 14 }, "#FF0000", 1, false)
  );

  selectedRegion.addEntity(
    new TileMarker(selectedRegion, { x: 36, y: 14 }, "#00FF00", 1, false)
  );
...
document
  ...
  .addEventListener("click", () =>
    world.isPaused ? world.startTicking() : world.stopTicking()
  );

ImageLoader.onAllImagesLoaded(() =>
  MapController.controller.updateOrbsMask(
    Viewport.viewport.player.currentStats,
    Viewport.viewport.player.stats
  )
);
