...
export class Mob extends Unit {
  ...
  movementStep() {
    ...
    if (this.canMove() && this.aggro) {
      let dx =
        this.location.x + Math.sign(this.aggro.location.x - this.location.x);
      let dy =
        this.location.y + Math.sign(this.aggro.location.y - this.location.y);
      ...
      let xSpace = every(
        xTiles.map((location: Location) =>
          Pathing.canTileBePathedTo(
            this.region,
            location.x,
            location.y,
            1,
            this.consumesSpace as Mob
          )
        ),
        Boolean
      );
