...
export class Settings {
  ...
  static persistToStorage() {
    ...
    window.localStorage.setItem(
      "displayXpDrops",
      String(Settings.displayXpDrops)
    );
    ...
    window.localStorage.setItem(
      "stats",
      SerializePlayerStats(Settings.player_stats)
    );
    ...
    window.localStorage.setItem(
      "displayPlayerLoS",
      String(Settings.displayPlayerLoS)
    );
    window.localStorage.setItem(
      "displayMobLoS",
      String(Settings.displayMobLoS)
    );
    window.localStorage.setItem("metronome", String(Settings.metronome));
    window.localStorage.setItem(
      "displayFeedback",
      String(Settings.displayFeedback)
    );
    window.localStorage.setItem(
      "tile_markers",
      JSON.stringify(Settings.tile_markers)
    );
  ...
  static readFromStorage() {
    ...
    Settings.westPillar =
      window.localStorage.getItem("westPillar") !== "false" || false;
    Settings.northPillar =
      window.localStorage.getItem("northPillar") !== "false" || false;
    Settings.displayPlayerLoS =
      window.localStorage.getItem("displayPlayerLoS") === "true" || false;
    Settings.displayMobLoS =
      window.localStorage.getItem("displayMobLoS") === "true" || false;
    Settings.lockPOV = false; //window.localStorage.getItem('lockPOV') !== 'false' || false;
    Settings.displayFeedback = !(
      window.localStorage.getItem("displayFeedback") === "false" || false
    );
    Settings.metronome =
      window.localStorage.getItem("metronome") === "true" || false;

    Settings.region = "inferno";
    Settings.displayXpDrops = !(
      window.localStorage.getItem("displayXpDrops") === "false" || false
    );

    Settings.inventory_key =
      window.localStorage.getItem("inventory_key") || "F4";
    Settings.spellbook_key =
      window.localStorage.getItem("spellbook_key") || "F2";
    Settings.equipment_key =
      window.localStorage.getItem("equipment_key") || "F1";
    ...
    Settings.player_stats = DeserializePlayerStats(
      window.localStorage.getItem("stats")
    );
    Settings.tile_markers = JSON.parse(
      window.localStorage.getItem("tile_markers")
    );
