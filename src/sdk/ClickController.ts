...
export class ClickController {
  ...
  unload() {
    ...
    this.viewport.canvas.removeEventListener(
      "mousemove",
      this.eventListeners[2]
    );
    this.viewport.canvas.removeEventListener(
      "mousemove",
      this.eventListeners[3]
    );
    this.viewport.canvas.removeEventListener(
      "mousemove",
  ...
  registerClickActions() {
    this.viewport.canvas.addEventListener(
      "mousedown",
      (this.eventListeners[0] = this.clickDown.bind(this))
    );
    this.viewport.canvas.addEventListener(
      "mouseup",
      (this.eventListeners[1] = this.leftClickUp.bind(this))
    );
    this.viewport.canvas.addEventListener(
      "mousemove",
      (this.eventListeners[2] = (e: MouseEvent) =>
        ControlPanelController.controller.cursorMovedTo(e))
    );
    this.viewport.canvas.addEventListener(
      "mousemove",
      (this.eventListeners[3] = (e: MouseEvent) =>
    ...
    this.viewport.canvas.addEventListener(
      "wheel",
      (this.eventListeners[6] = this.wheel.bind(this))
    );
  }
  ...
  leftClickUp(e: MouseEvent) {
    ...
    const intercepted =
      ControlPanelController.controller.controlPanelClickUp(e);
    if (intercepted) {
      return;
    }
  }
  ...
  rightClickDown(e: MouseEvent) {
    ...
    if (e.offsetX > width - ControlPanelController.controller.width) {
      if (
        ...
        this.viewport.height * Settings.tileSize -
          ControlPanelController.controller.height
      ) {
        const intercepted =
          ControlPanelController.controller.controlPanelRightClick(e);
        if (intercepted) {
          return;
        }
      }
    ...
    groundItems.forEach((item: Item) => {
      menuOptions.push({
        ...
        action: () =>
          this.sendToServer(() =>
            Viewport.viewport.player.setSeekingItem(item)
          ),
      });
    });
      ...
      },
      {
        ...
        action: () => {
          ...
          entitiesAtPoint.forEach((entity: TileMarker) => {
            if (
              entity.entityName() === EntityName.TILE_MARKER &&
              entity.saveable
            ) {
          ...
          if (!removed) {
            region.addEntity(
              new TileMarker(
                Viewport.viewport.player.region,
                { x, y },
                "#FF0000"
              )
            );
          ...
          Settings.tile_markers = filter(
            filter(
              region.entities,
              (entity: Entity) => entity.entityName() === EntityName.TILE_MARKER
            ),
