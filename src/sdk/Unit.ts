...
export class Unit extends Renderable {
  ...
  get combatLevel() {
    const base =
      ...
      (this.stats.defence +
        this.stats.hitpoint +
        Math.floor((this.stats.prayer || 0) * 0.5));
  ...
  combatLevelColor(against: Unit) {
    ...
    const difference = Math.min(
      10,
      Math.max(-10, theirCombatLevel - myCombatLevel)
    );
  ...
  static mergeEquipmentBonuses(
    firstBonuses: UnitBonuses,
    secondBonuses: UnitBonuses
  ): UnitBonuses {
    return {
      attack: {
      ...
      other: {
        meleeStrength:
          firstBonuses.other.meleeStrength + secondBonuses.other.meleeStrength,
        rangedStrength:
          firstBonuses.other.rangedStrength +
          secondBonuses.other.rangedStrength,
        magicDamage:
          firstBonuses.other.magicDamage + secondBonuses.other.magicDamage,
        prayer: firstBonuses.other.prayer + secondBonuses.other.prayer,
        crystalAccuracy:
          (firstBonuses.other.crystalAccuracy || 0) +
          (secondBonuses.other.crystalAccuracy || 0),
        crystalDamage:
          (firstBonuses.other.crystalDamage || 0) +
          (secondBonuses.other.crystalDamage || 0),
      },
      targetSpecific: {
        undead:
          firstBonuses.targetSpecific.undead +
          secondBonuses.targetSpecific.undead,
        slayer:
          firstBonuses.targetSpecific.slayer +
          secondBonuses.targetSpecific.slayer,
      },
    };
  }
  ...
  canMove() {
    return (
      !this.hasLOS && !this.isFrozen() && !this.isStunned() && !this.isDying()
    );
  }
  ...
  setHasLOS() {
    ...
    if (this.aggro.type === UnitTypes.PLAYER) {
      ...
      this.aggro.type === UnitTypes.MOB
    ) {
      this.hasLOS = LineOfSight.mobHasLineOfSightToMob(
        this.region,
        this,
        this.aggro,
        this.attackRange
      ...
      if (projectile.remainingDelay === 0) {
        ...
        if (projectile.damage < 0) {
          ...
          if (this.currentStats.hitpoint < this.stats.hitpoint) {
            ...
            this.currentStats.hitpoint = Math.min(
  ...
  drawHitsplats(context: OffscreenCanvasRenderingContext2D, scale: number) {
    ...
    this.incomingProjectiles.forEach((projectile) => {
      ...
      if (this.drawHitsplat(projectile)) {
        let image =
          projectile.damage === 0
            ? this.missedHitsplatImage
            : this.damageHitsplatImage;
        ...
        projectileOffsets = remove(projectileOffsets, (offset) => {
          return (
            offset[0] !== projectile.offsetX || offset[1] !== projectile.offsetY
          );
        });
